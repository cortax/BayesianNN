...
def full_features_lstm_training(index_dataset, experiment_number, amount_datasets,
                    n_epoch, batch_size, sampler_type,
                    early_stop, patience, cooldown, factor, learning_rate, verbose, threshold,
                    features_amount, labels_type, size_windows, normalize,
    ...
    folder_string = 'featuresLSTMModel/{}/experiment{}'.format(dataset_name_string, experiment_number)
    file_name_string = 'subDatasetNumber_{}of{}_TODO'.format(index_dataset+1, amount_datasets)
    
    print('Making Dataset')
    list_train_dataset, list_valid_dataset, list_test_dataset, list_data_storage = get_multiple_features_multiple_datasets(
    ...
    print('Making Model')
    model = IndexLSTMModel(features_amount, labels_type, size_windows,
    ...
    if labels_type != 1:
        ...
        list_labels = ['0', '1', '2', '3']
        confMat_title = 'Confusion Matrix for LSTM Base Model on Test Set #{}'.format(index_dataset)
...
def train_features_lstm_main(experiment_number, amount_datasets,
                            n_epoch, batch_size, sampler_type,
                            early_stop, patience, cooldown, factor, learning_rate, verbose, threshold,
                            features_amount, labels_type, size_windows, normalize,
                            split_train_test_ratio, split_train_valid_ratio,
    ...
    for index_dataset in range(amount_datasets):
        #print(index_dataset)
        test_loss, test_metrics = full_features_lstm_training(index_dataset, experiment_number, amount_datasets,
                                            n_epoch, batch_size, sampler_type,
                                            early_stop, patience, cooldown, factor, learning_rate, verbose, threshold,
                                            features_amount, labels_type, size_windows, normalize,
                                            split_train_test_ratio, split_train_valid_ratio,
