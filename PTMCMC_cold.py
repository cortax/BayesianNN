...
def main(tag, nb_chunk, chunk_size):
    ...
    for s in range(nb_chunk):
        print(s)
        if os.path.exists("Results/PTMCMC_cold/PTMCMC_checkpoint_xp_" + tag + "_chunk_" + str(s) + ".pt"):
            (x, _, _, _) = pickle.load( open("./Results/PTMCMC_cold/PTMCMC_checkpoint_xp_" + tag + "_chunk_" + str(s) + ".pt", "rb" ) )
        else:
            x, ladderAcceptanceRate, swapAcceptanceRate, logProba = sampler.run(chunk_size)
            pickle_out = open("Results/PTMCMC_cold/PTMCMC_checkpoint_xp_" + tag + "_chunk_" + str(s) + ".pt","wb")
