...
class AbstractRegressionSetup(ABC):
    ...
    def evaluate_metrics(self, theta,device):
        ...
        nLPP_train = nLPP(self._loglikelihood, theta, self._X_train, self._y_train,device)
        nLPP_validation = nLPP(self._loglikelihood, theta, self._X_validation, self._y_validation,device)
        nLPP_test = nLPP(self._loglikelihood, theta, self._X_test, self._y_test,device)

        RSE_train = RSE(self._normalized_prediction, theta, self._X_train, self._y_train,device)
        RSE_validation = RSE(self._normalized_prediction, theta, self._X_validation, self._y_validation,device)
        RSE_test = RSE(self._normalized_prediction, theta, self._X_test, self._y_test,device)
