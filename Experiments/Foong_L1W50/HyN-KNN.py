...
class HyNetEns(nn.Module):
    ...
    def KNNE(self,nb_samples,k=1):  
        ...
        lcd = (d/2.0*torch.log(torch.tensor(math.pi))) - torch.lgamma(torch.tensor(1.0 + d/2.0)).to(device)
        return torch.digamma(torch.tensor(nb_samples).float()).double() - torch.digamma(torch.tensor(k).float()).double() + lcd + d/nb_samples*torch.sum(torch.log(a)).to(device)
