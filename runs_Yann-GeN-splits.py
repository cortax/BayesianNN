...
if __name__ == "__main__":
    ...
    dataset='powerplant'
    print(dataset)
    metrics, models=run(dataset, batch=500) 
    print(dataset+': done :-)')
    ...
    for dataset in ['boston', 'yacht', 'concrete','energy', 'wine']:
        print(dataset)
        metrics, models =run(dataset, batch=None) 
        print(dataset+': done :-)')
        results.update(metrics)
        Models.update({dataset:models})


    torch.save(results, 'Results/GeN_splits.pt')
    torch.save(Models, 'Results/GeN_splits_Models.pt')
